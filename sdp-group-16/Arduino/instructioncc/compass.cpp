...
int compass_debug=0; // ) for normal run, 1 for long output, 2 for self-calibration
int calibration_duration = 15000;
...
// Manual turning for self-calibration
void turn() {
  motorMove(FR_MOTOR_IDX, 100, MOTOR_FWD);
  motorMove(FL_MOTOR_IDX, 100, MOTOR_BWD);
  motorMove(BR_MOTOR_IDX, 100, MOTOR_BWD);
  motorMove(BL_MOTOR_IDX, 100, MOTOR_FWD);
}
...
void compass_offset_calibration(int select){
 ...
 // Configuring the Control register for Positive Bais mode
  if (compass_debug == 1) {
    Serial.println(F("Calibrating the Magnetometer ....... Gain"));
  }
  Wire.beginTransmission(compass_address);
  Wire.write(0x00);
  ...
  // *****************************************************************************************
   if (select == 2 | select == 3){// User input in the function
    if (compass_debug == 1) {
      Serial.println(F("Calibrating the Magnetometer ....... Offset"));
      Serial.println(F("Please rotate the magnetometer 2 or 3 times in complete circules with in one minute ............. "));
    }
    ...
    */
    if (compass_debug == 1){
      Serial.println(F("Starting Debug data in "));
      delay(1000);
      Serial.println(F("3"));
      delay(1000);
      Serial.println(F("2"));
      ...
      Serial.println(F("*"));
      Serial.println(F("Debug -- (Offset Calibration)"));
      for(byte i=0;i<10;i++){  
         Serial.print(F("*"));
      }
      Serial.println(F("*"));
    ...
    while(millis()-t <= calibration_duration){
    ...
    */
    if (compass_debug == 1){
      ...
      Serial.println(F("*"));
      Serial.println(F("End Debug -- (Offset Calibration)"));
      for(byte i=0;i<10;i++){   
         Serial.print(F("*"));
      }
      Serial.println(F("*"));
...
void compass_init(int gain){
  ...
  if (compass_debug == 1) {
    Serial.print(F("Gain updated to  = "));
    Serial.print(compass_gain_fact);
    Serial.println(F(" mG/bit"));
  }
